 ...
 */
function Spec(e){function t(){return Array(r).join("  ")}Base.call(this,e);var n=this,r=(this.stats,0),i=0;e.on("start",function(){console.log()}),e.on("suite",function(e){++r,console.log(color("suite","%s%s"),t(),e.title)}),e.on("suite end",function(e){--r,1==r&&console.log()}),e.on("pending",function(e){var n=t()+color("pending","  - %s");console.log(n,e.title)}),e.on("pass",function(e){if("fast"==e.speed){var n=t()+color("checkmark","  "+Base.symbols.ok)+color("pass"," %s ");cursor.CR(),console.log(n,e.title)}else{var n=t()+color("checkmark","  "+Base.symbols.ok)+color("pass"," %s ")+color(e.speed,"(%dms)");cursor.CR(),console.log(n,e.title,e.duration)}}),e.on("fail",function(e,n){cursor.CR(),console.log(t()+color("fail","  %d) %s"),++i,e.title)}),e.on("end",n.epilogue.bind(n))}var Base=require("./base"),cursor=Base.cursor,color=Base.color;exports=module.exports=Spec,Spec.prototype.__proto__=Base.prototype;